...
const FindDriverPage = () => {
  ...
  const findAndAssignDriver = async (currentBookingId) => {
    try {
      ...
      // Get fresh booking data from Firestore
      const bookingRef = doc(db, 'airportTransfers', currentBookingId);
      ...
      // CRITICAL: HARD TERMINAL STATE - If driver is already accepted, stop all processing
      if (data.status === 'accepted' || data.driverId) {
        console.log('✅ Driver already accepted - stopping search');
        clearAllTimers();
        return;
      }
  ...
  const startPeriodicDriverSearch = (bookingId) => {
    ...
    periodicCheckTimer.current = setInterval(() => {
      // Check Firestore for the latest status before each search
      const bookingRef = doc(db, 'airportTransfers', bookingId);
      getDoc(bookingRef).then(bookingSnap => {
        if (bookingSnap.exists()) {
          const data = bookingSnap.data();
          // Only continue searching if status is still searching and no driver assigned
          if (data.status === 'searching_driver' && !data.driverId) {
            ...
            console.log('⏹️ Driver found - stopping periodic checks');
  ...
  useEffect(() => {
    ...
    // Repeat announcement every 30 seconds while searching
    ...
    const createBookingAndStartSearch = async () => {
      try {
        ...
        try {
          // Create booking
          const docRef = await addDoc(collection(db, 'airportTransfers'), bookingData);
          ...
          if (isMounted) {
            ...
            // Start the driver search
            startPeriodicDriverSearch(newBookingId);
          }
        ...
        console.error('Error in createBookingAndStartSearch:', error);
    ...
    // Cleanup function to unsubscribe from listener and prevent state updates after unmount
    return () => {
      isMounted = false;
  ...
  }, [state?.bookingDetails, navigate]);
  
  // Clean up all timers and listeners when component unmounts
  useEffect(() => {
    return () => {
      clearAllTimers();
    };
